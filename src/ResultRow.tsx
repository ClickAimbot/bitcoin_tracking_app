import paybisLogo from './assets/paybislogo.png';
...
export default function ResultRow({loading, providerName, btc}:ResultRowProps) {
    return (
        <a 
            ...
            target='_blank'
            className="block relative border min-h-[64px] border-white/10 rounded-lg 
                bg-gradient-to-r from-purple-500/10 to-blue-500/10 p-4 my-2 overflow-hidden">
            <div className="flex gap-4">
                {providerName && (
                  <div className='grow items items-center flex'>
                    <img 
                ...
                {btc && (
                    <div className="flex gap-2">
                        <span className="text-xl text-purple-200/80">
                            { new Intl.NumberFormat('en-US', {minimumFractionDigits:8}).format(Number(btc)) }
                        </span>
            ...
            {loading && (
                <div className="inset-0 absolute bg-gradient-to-r from-transparent via-red-500 to-transparent
                skeleton-animation border-t border-white/25"
                 />
            )}  
        </a>
    )
